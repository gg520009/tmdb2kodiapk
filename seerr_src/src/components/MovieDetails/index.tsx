    ...
    >
      ...
      <div className="media-header">
        ...
        <div className="media-actions">
          ...
          <button
            type="button"
            className="z-40 ml-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold px-4 py-2 rounded-lg transition"
            onClick={() => {
              if (typeof window !== 'undefined') {
                window.dispatchEvent(
                  new CustomEvent('openKodiBridge', {
                    detail: {
                      mediaType: 'movie',
                      tmdbId: data.id,
                      title: data.title,
          ...
          <button
            type="button"
            className="z-40 ml-2 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold px-4 py-2 rounded-lg transition"
            onClick={async () => {
              if (typeof window !== 'undefined') {
                try {
                  const res = await fetch(`/api/v1/bridge/resolve?type=movie&tmdbId=${data.id}`);
                  const json = await res.json();
                  if (json.streams && json.streams.length > 0) {
                    window.open(json.streams[0].url, '_blank');
                  } else {
                    window.open(`https://www.fmovies.gd/movie/${data.id}`, '_blank');
                  }
                } catch (e) {
                  window.open(`https://www.fmovies.gd/movie/${data.id}`, '_blank');
                }
